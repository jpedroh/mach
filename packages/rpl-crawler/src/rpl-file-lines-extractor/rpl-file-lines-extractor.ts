...
const makeRplFileLinesExtractor = ({ zip }: { zip: ZipPort }) => {
    return (buffer: Buffer) => {
        ...
        const flightsFileName = zipFile.listZipFileNames().find(fileName => fileName.includes('EOBT'))

        return zipFile.readFileLines(flightsFileName).filter(line => line.includes('EQPT'))
    }
}
