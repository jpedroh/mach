...
test('if there are no results for the search query it shows a friendly message', async ({
  ...
  await expect(page.getByText(/there are no results for your search/i)).toBeVisible()
})

test('if there are results for the search it shows them on the table', async ({ page }) => {
...
test('when i click in view details it shows me the flight details modal', async ({ page }) => {
